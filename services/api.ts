...
import { AuthResponse, LoginData, Message, RegisterData } from "../types";
...
class ApiService {
  ...
  async sendMessage(username: string, message: string): Promise<Message> {
    const response = await this.api.post("/messages", { username, message });
    return response.data.data;
  }
